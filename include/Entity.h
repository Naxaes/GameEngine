...
public:
    ...
    {
        unsigned int component_id = get_component_id<T>();
        component_mask[component_id] = true;

        Component* component = new T(this, std::forward<TemplateArgs>(args)...);
        components[component_id] = std::move(component);

        return *static_cast<T*>(components[component_id]);
    };
    ...
    {
        unsigned int component_id = get_component_id<T>();
        component_mask[component_id] = false;
        components[component_id] = nullptr;
    }
    ...
    {
        unsigned int component_id = get_component_id<T>();
        bool exists = component_mask[component_id];
...
private:
    std::array<Component*, MAX_COMPONENTS> components{nullptr};
...
// This would be nice and efficient, but it's too complicated at the moment to be worth it.
#if 0
...
public:
    ...
    // Of course, the get_component_id function have to change (maybe just put them as manager methods).
