...
GLFWwindow* create_window(unsigned short width, unsigned short height, const char* title)
{
    ...
    initialize();
    add_hints();     // HINTS: http://www.glfw.org/docs/latest/window_guide.html#window_hints
...
Window::Window(unsigned short width, unsigned short height, const char* title)
{
    ...
    ASSERT(handle, "Failed to create GLFW window.");
}
...
std::string Window::version()
{
    return "GLFW version " + std::string(glfwGetVersionString());
}
...
void Window::clear()
{
    GLCALL(glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT));  // Should be abstracted out to the renderer.
}